...
const getDeploymentPrompt = (preset) =>
  Array.isArray(preset.contract.prompts)
    ? preset.contract.prompts.find((prompt) => prompt.name === DEPLOYMENT_PROMPT_NAME)
    : null;
...
const resolveDefaultDeploymentTarget = (preset) => {
  ...
  return options.includes(deploymentPrompt?.default) ? deploymentPrompt.default : fallback;
};
...
const runInit = async () => {
  ...
  const templateRoot = resolveTemplateRoot(preset.indexEntry.templateRoot, mode);
